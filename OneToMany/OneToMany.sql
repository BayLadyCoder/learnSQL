...
----------------------------------------------------------------------------------------
ON DELETE CASCADE
-- allow us to delete customers and automatically delete all the orders which have FOREIGN KEY associate with the customers

-- Like this example, we cannot drop customers table before we drop orders table
DROP TABLE orders, customers;
...
-- Add ON DELETE CASCADE under FOREIGN KEY and REFERENCES
CREATE TABLE orders (
    id INT AUTO_INCREMENT PRIMARY KEY,
    order_date DATE,
    amount DECIMAL(8,2),
...
-- We are going to delete BOY GEORGE (from customers table) and it will delete all Boy George's orders (from orders table) automatically
